/**
 * 216 ms, faster than 79.23%
 */
var largestWordCount = function(messages, senders) {
  let table = {};
  for (let i = 0; i < messages.length; i++) {
    const c = messages[i].split(' ').length;
    if (table[senders[i]] === undefined) {
      table[senders[i]] = [senders[i], c];
    } else {
      table[senders[i]][1] += c;
    }
  }
